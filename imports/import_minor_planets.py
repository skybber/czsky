...
def _int_designation_from_mpc_row(mpc_mp):
    designation = str(mpc_mp['designation'] or '')
    match = re.match(r'^\((\d+)\)', designation)
    if match:
        return int(match.group(1))
    mpc_designation = str(mpc_mp['designation_packed']).strip()
    try:
        return int(mpc_designation)
    except (TypeError, ValueError):
        return None


def assign_minor_planet_from_mpc_row(minor_planet, mpc_mp):
    mpc_designation = str(mpc_mp['designation_packed']).strip()
    minor_planet.mpc_designation = mpc_designation
    minor_planet.int_designation = _int_designation_from_mpc_row(mpc_mp)
    ...
    with load.open(mpcorb_file) as f:
        ...
        for index, mpc_mp in all_minor_planets.iterrows():
            ...
            if minor_planet is None:
                int_designation = _int_designation_from_mpc_row(mpc_mp)
                minor_planet = MinorPlanet.query.filter_by(int_designation=int_designation).first() if int_designation else None
            if minor_planet is None:
                minor_planet = MinorPlanet()
            assign_minor_planet_from_mpc_row(minor_planet, mpc_mp)
